...
manifoldAlignment <- function(X, Y, d = 2, lambda = 0.9, k = 2, gamma = 0.5, methoduse = 1){
  ...
  W = diag(Dcol) - W * (Drow^(-1)) * Dcol
  W = t(W) %*% W
  E = RSpectra::eigs(W, d*2, 'SR', maxitr = 2000, tol = 1e-5)
  E <- suppressWarnings(RSpectra::eigs(W, d*2, 'SR'))
  E$values <- suppressWarnings(as.numeric(E$values))
  E$vectors <- suppressWarnings(apply(E$vectors,2,as.numeric))
  newOrder <- order(E$values)
...
set.seed(1)
MA <- manifoldAlignment(WT, KO, d = 3, lambda = 0.9, k = 3, gamma = 1, methoduse = 1)
set.seed(1)
...
saveRDS(outputList, "SRS3059998_yan_k3_gamma1_method1.rds")

set.seed(1)
MA <- manifoldAlignment(WT, KO, d = 3, lambda = 0.9, k = 3, gamma = 1, methoduse = 0)
set.seed(1)
DR <- dRegulation(MA, gKO = 'Mecp2')
outputList <- list()
outputList$WT <- WT
...
saveRDS(outputList, "SRS3059998_yan_k3_gamma1_method0.rds")

set.seed(1)
MA <- manifoldAlignment(WT, KO, d = 3, lambda = 0.9, k = 3, gamma = 0.5, methoduse = 1)
set.seed(1)
DR <- dRegulation(MA, gKO = 'Mecp2')
outputList <- list()
outputList$WT <- WT
...
saveRDS(outputList, "SRS3059998_yan_k3_gamma0_method1.rds")

set.seed(1)
MA <- manifoldAlignment(WT, KO, d = 3, lambda = 0.9, k = 3, gamma = 0.5, methoduse = 0)
set.seed(1)
DR <- dRegulation(MA, gKO = 'Mecp2')
outputList <- list()
outputList$WT <- WT
...
saveRDS(outputList, "SRS3059998_yan_k3_gamma0_method0.rds")
...
saveRDS(outputList, "SRS3059999_yan_k3_gamma1_method1.rds")
...
saveRDS(outputList, "SRS3059999_yan_k3_gamma1_method0.rds")
...
saveRDS(outputList, "SRS3059999_yan_k3_gamma0_method1.rds")
...
saveRDS(outputList, "SRS3059999_yan_k3_gamma0_method0.rds")