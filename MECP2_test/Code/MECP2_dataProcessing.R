...
load('MECP2_test/Results/SRS3059998.RData')
load('MECP2_test/Results/SRS3059999.RData')
...
png(filename = 'MECP2_test/Results/tf_MECP2.png',width = 750, height = 500, res = 300)
...
## check intesection
png('MECP2_test/Results/upset_MECP2.png', width = 1300, height = 1500, res = 300)
...
png('MECP2_test/Results/cl_MECP2.png', width = 1800, height = 1000, res = 300, pointsize = 20)
...
png('MECP2_test/Results/ego_SRS3059998.png', width = 6000, height = 6000, pointsize = 15, res=300, bg = NA)
par(mar=c(0,0,0,0))
plot(netPlot,
         layout = layPlot,
         edge.arrow.size=.2,
...
sharedColor <- hcl.colors(5, palette = 'Zissou 1', alpha = 0.5)[5]
vColor <- ifelse(names(V(netPlot)) %in% sharedGene, sharedColor, uniqueColor)
png('MECP2_test/Results/ego_SRS3059999.png', width = 6000, height = 6000, pointsize = 15, res=300, bg = NA)
par(mar=c(0,0,0,0))
plot(netPlot,
     layout = layPlot,
     edge.arrow.size=.2,
...
png('MECP2_test/Results/zScores_MECP2.png', width = 1800, height = 1500, res = 300, pointsize = 20)
...
B <- plotEnrichment(WP$`Brain-Derived Neurotrophic Factor (BDNF) signaling pathway WP2380`, stats = zSRS3059999) +
  ...
  labs(title = 'SRS3059999\nBDNF signaling pathway',
       subtitle = paste0('FDR = ', formatC(E[grepl('\\(BDNF\\)',E$pathway),]$padj, digits = 2, format = 'e')))
png('MECP2_test/Results/bndf_SRS3059998.png', width = 1000, height = 750, res = 300)
A
dev.off()
png('MECP2_test/Results/bndf_SRS3059999.png', width = 1000, height = 750, res = 300)
B
dev.off()
