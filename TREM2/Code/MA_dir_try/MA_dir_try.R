...
KO <- WT
KO[, gKO] <- 0
MA_col_nnorm <- manifoldAlignment_nnorm(X = WT, Y = KO, d = 50)
saveRDS(MA_col_nnorm, "MA_col_nnorm.rds")
# both 0
KO <- WT
KO[gKO, ] <- 0
KO[, gKO] <- 0
MA_both_nnorm <- manifoldAlignment_nnorm(X = WT, Y = KO, d = 50)
saveRDS(MA_both_nnorm, "MA_both_nnorm.rds")

## normalized
# row 0 
KO <- WT
KO[gKO, ] <- 0
MA_row_norm <- manifoldAlignment_norm(X = WT, Y = KO, d = 50)
saveRDS(MA_row_norm, "MA_row_norm.rds")
# col 0 
KO <- WT
KO[, gKO] <- 0
MA_col_norm <- manifoldAlignment_norm(X = WT, Y = KO, d = 50)
saveRDS(MA_col_norm, "MA_col_norm.rds")
# both 0
KO <- WT
KO[gKO, ] <- 0
KO[, gKO] <- 0
MA_both_norm <- manifoldAlignment_norm(X = WT, Y = KO, d = 50)
saveRDS(MA_both_norm, "MA_both_norm.rds")
